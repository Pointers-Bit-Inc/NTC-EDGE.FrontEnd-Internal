...
export function getStatusText(props: any, personnel: UserApplication | undefined) {
    ...
    return (props?.service?.serviceCode == 'service-22' && props?.status == FORAPPROVAL) ? APPROVED : getRole(props.user, [EVALUATOR, DIRECTOR, ADMIN]) && getStatus(props, personnel) == FORAPPROVAL && (
...
export function getActivityStatus(props: any, status: string) {
    ...
    return [CASHIER].indexOf(props?.role) != -1 ? PaymentStatusText(props?.activity?.paymentStatus || (props?.activity?.service?.serviceCode == 'service-22' && props?.activity?.status != PENDING ? APPROVED : FORVERIFICATION)) : StatusText((props?.activity?.service?.serviceCode == 'service-22' && props?.activity?.status == FORAPPROVAL ? APPROVED : (props?.activity?.status == FORAPPROVAL && props.activity.assignedPersonnel?.role != props.currentUser?.role?.key ? APPROVED : props?.activity?.status)) || (props?.activity?.service?.serviceCode == 'service-22' ? APPROVED : FORAPPROVAL))
}
