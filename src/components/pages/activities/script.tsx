...
export function getStatusText(props: any , personnel: UserApplication | undefined) {
    return getRole(props.user , [EVALUATOR , DIRECTOR]) && getStatus(props , personnel) == FORAPPROVAL && (
        ...
        props?.activity?.approvalHistory?.[0]?.status !== FORAPPROVAL && props?.activity?.approvalHistory?.status !== FORAPPROVAL || (props?.activity?.assignedPersonnel?._id !== props.currentUser?._id))  ? StatusText(APPROVED) : getRole(props.user , [ACCOUNTANT]) && !!props.paymentMethod && (
