            ...
            <SafeAreaView style={{flex: 1, backgroundColor: primaryColor}}>
                <View style={{backgroundColor:"#F8F8F8",flex:1,flexDirection:"row"}}>
                    <View onLayout={onActivityLayoutComponent} style={[styles.container,styles.shadow,{
                        ...
                        <Animated.View style={[!modalVisible&& !moreModalVisible&& !visible&& !refreshing&& !lodash.size(meetingList)&& {top: containerHeight, zIndex:10,transform:[{translateY:headerTranslate}], }, {paddingVertical: 3,  backgroundColor: "#fff", flexDirection: "row", justifyContent: "space-around", alignItems: "center"}]}>
                            {tabs.map((tab) => {
                                return <TouchableOpacity onPress={() => {
                                    setSwitchTab(tab.id)
                                }}>
                                    <View style={{paddingVertical: 6, borderBottomWidth: switchTab == tab.id ? 3 : 0, borderBottomColor:switchTab == tab.id ? primaryColor : "rgba(255, 255, 255, 0)"}} >
                                        <Text style={{fontSize: fontValue(14), fontFamily: Regular500}}>{tab.name}</Text>
                                    </View>
                                </TouchableOpacity>
                            })}
                        ...
                        <Animated.FlatList
                            ...
                            ListEmptyComponent={()=>listEmpty(refreshing,searchTerm,( switchTab == 0 || switchTab == 2 ?  notPnApplications.length +  pnApplications?.map((item:any,index:number)=>item?.activity&&item?.activity?.map((act:any,i:number)=>(
                                    act?.assignedPersonnel?._id||act?.assignedPersonnel)==user?._id)).length:
                                switchTab == 1 ? pnApplications?.map((item:any,index:number)=>item?.activity&&item?.activity?.map((act:any,i:number)=>(
                                    act?.assignedPersonnel?._id||act?.assignedPersonnel)==user?._id)).length : notPnApplications.length))}
                            ListHeaderComponent={ !(switchTab == 1 || switchTab == 2)  ? listHeaderComponent() : null}

                            style={{flex:1,}}
                            ref={flatListViewRef}
                            data={switchTab == 1 ? pnApplications : notPnApplications}
