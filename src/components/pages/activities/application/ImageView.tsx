...
import {findNodeHandle , Image , Modal , PanResponder , View ,} from "react-native";
...
const updateOverlayFrame = (prePinch , currPinch , overlayLayout) => {
    ...
    x += 1 * (
        currPinch[0].pageX - prePinch[0].pageX);
    y += 1 * (
...
const ImageView = ({
    ...
    const hideOverlay = () => {
        ...
        let dx = (
            current.x - initial.x) / HIDING_FRAMES_COUNT;
        let dy = (
            current.y - initial.y) / HIDING_FRAMES_COUNT;
        let dw = (
            current.w - initial.w) / HIDING_FRAMES_COUNT;
        let dh = (
            current.h - initial.h) / HIDING_FRAMES_COUNT;
        let ctr = 0;
        const updateOverlay = () => {
            ++ctr;
