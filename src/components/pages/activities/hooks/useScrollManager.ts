...
export const useScrollManager = (routes: { key: string; title: string }[], sizing = Theme.sizing, headerHeight) => {
  const CBTabViewOffset = Platform.OS === "ios" ? -Theme.sizing.header : 0
  const scrollY = useRef(new Animated.Value(-Theme.sizing.header)).current;
    ...
    const syncScrollOffset = () => {
      ...
      Object.keys(tabkeyToScrollableChildRef).map((key) => {
        ...
        if (/* header visible */ key !== curRouteKey) {
          if (scrollValue <= CBTabViewOffset + Theme.sizing.header) {
            scrollRef.scrollToOffset({
              offset: Math.max(
                Math.min(scrollValue, CBTabViewOffset + Theme.sizing.header),
                CBTabViewOffset,
              ),
              animated: false,
            });
            ...
            scrollRef.scrollToOffset({
              offset: CBTabViewOffset + Theme.sizing.header,
              animated: false,
            });
            tabkeyToScrollPosition[key] =
              CBTabViewOffset + Theme.sizing.header ;
