...
export default ({
    ...
    const settings: string | ArrayLike<any> | null | undefined = [
        /*{
          label: 'Notifications',
          value: 'notifications',
          disabled: true,
            ...
            <TouchableOpacity disabled={item?.disabled ? true : false} onPress={item?.onPress}>
                <View style={[styles?.row, {justifyContent: 'space-between'}]}>
                    <View style={styles?.row}>
                        {item?.icon}
                        <Text
                            style={[styles.textSettings, !!item?.disabled && {color: disabledColor}, item?.value === 'logout' && {color: text.error}]}>{item?.label}</Text>
                    </View>
                    {
