...
const Preview: FC<Props> = ({
    ...
    const renderStatus = () => {
        let { approvalHistory = {}, paymentHistory = {}, remarks = '' } = myApplication || {};
        ...
        return (
            <View style={styles.tableContainer}>
                ...
                {
                    !!remarks &&
                    <View style={[styles?.remarksContainer, {borderColor: statusColor()}]}>
                        <Text style={styles?.remarksTitle}>{status() === 'Declined' ? 'NOD/' : ''}Remarks</Text>
                        <Text style={[styles?.remarksContent, {color: statusColor()}]}>{remarks}</Text>
                    </View>
                }
            </View>
        )
    ...
    const renderGrandchild = ({item, index, parentId}: any) => {
        ...
        if (item?.type === 'date') {
            ...
            value = Moment(`${year}-${month}-${day}`).format('LL');
        }
        if (item?.type === 'unit') {
        ...
        else if (typeof(value) === 'string') return renderRow(item?.label, value);
        else return <></>;
    };
