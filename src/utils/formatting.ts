...
export const RNValue = (number: number) => {
    ...
    if (service) {
        ...
        Object.keys(service)?.forEach((parent, index) => {
            if (
                ...
                const _specifyML = {
                    /**will be temporarily not prefilled-in */
                    id: 'vehiclePlateNo',
                    label: 'Plate No.',
                ...
                if (parentItem?.length > 0 && form_parentIndex > -1) { // parentList, particulars ex. && field exists on form
                        ...
                        Object.keys(child)?.forEach((grandchild, index) => {
                            ...
                            if (typeof (grandchildItem) === 'string') { // stationClass ex.
                                ...
                                if (!!formPath) {
                                    ...
                                    if (type === 'date') {
                                        let y = extractDate(grandchildItem, 'year'),
                                            m = extractDate(grandchildItem, 'month'),
                                            d = extractDate(grandchildItem, 'date');
                                        ...
                                        let isValid = validate && typeof validate === 'function'
                                            ? validate(grandchildItem)
                                            : validateText(grandchildItem);
                                ...
                                grandchildItem?.forEach((ggrandchild: any, index: number) => {
                                    ...
                                    Object.keys(ggrandchild)?.forEach((gggrandchild: any, index: number) => {
                                        ...
                                        if (!!formPath) {
                                            ...
                                            if (type === 'date') {
                                                let y = extractDate(gggrandchildItem, 'year'),
                                                    m = extractDate(gggrandchildItem, 'month'),
                                                    d = extractDate(gggrandchildItem, 'date');
                                ...
                                Object.keys(grandchildItem)?.forEach((ggrandchild, index) => {
                                    ...
                                    if (!!formPath) {
                                        ...
                                        if (type === 'date') {
                                            let y = extractDate(ggrandchildItem, 'year'),
                                                m = extractDate(ggrandchildItem, 'month'),
                                                d = extractDate(ggrandchildItem, 'date');
                                            ...
                                            let isValid = validate && typeof validate === 'function'
                                                ? validate(ggrandchildItem)
                                                : validateText(ggrandchildItem);
                    ...
                    Object.keys(parentItem)?.forEach((child, index) => {
                        ...
                        if (typeof parentItem?.[child] == "string") {
    ...
    const fillDocu = (type: string) => {
        ...
        if (docuId > -1) {
            ...
            if (expiryId > -1 && !!validUntil) {
                let y = extractDate(validUntil, 'year'), m = extractDate(validUntil, 'month'),
                    d = extractDate(validUntil, 'date');
